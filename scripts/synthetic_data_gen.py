...
def _induce_noise(x):
    noise_index = list(dict.fromkeys([random.randrange(1, len(x)) for _ in range(int(len(x) * 0.15))]))
    new_x = [0] * len(x)
...
def gen_desc(n_instances, sub_categories, file_name='synthetic_discrete.csv'):
    """
...
def gen_dataset(n_instances=1000, file_name='synthetic.csv'):
    try:
        os.mkdir('../dataset/')
...
# gen_dataset(n_instances=10000, file_name='synthetic2.csv')
